...
public class rail : MonoBehaviour
{
    ...
    public Vector3 positionOnRail(int reg, float ratio, playMod mode)
    {
        switch (mode)
        {
            default:
            case playMod.linear:
                return linearPosiion(reg, ratio);
            case playMod.catmull:
                return catmullPosition(reg, ratio);
        }
    }

    public Vector3 linearPosiion(int seg, float ratio)
    {
